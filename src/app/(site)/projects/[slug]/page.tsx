...
export async function generateMetadata({ params }: Props) {
    ...
    const project = getProjectsBySlug(slug, ["title","titlemeta", "keywords", "descriptionmeta", "ScopeOfWork", "industry", "raised", "website", "description", "heroImage", "gallery", "content"]);
    ...
    if (project) {
        const metadata = {
             title: `${project.titlemeta}`,
            description: `${project.descriptionmeta}`,
            keywords: `${project.keywords}`,
            alternates: {
                canonical: `https://sakne.ru/projects/${slug}`,
            },
            openGraph: {
                title: `${project.titlemeta}`,
                description: `${project.descriptionmeta}`,
                url: `https://sakne.ru/projects/${slug}`,
                images: [
                    {
...
export default async function Post({ params }: Props) {
    ...
    const project = getProjectsBySlug(slug, [
        "title", "ScopeOfWork", "industry", "raised", "website", "description", "heroImage", "gallery", "content"
    ]);
                            ...
                            <div className="flex flex-col gap-10">
                                <div>
                                    <Link href="/projects" className="group flex gap-3 items-center w-fit bg-primary hover:bg-secondary dark:border dark:border-primary dark:hover:border dark:hover:border-white/30 rounded-full transition-all duration-500 ease-in-out">
                                        ...
                                        <span className="pr-4 text-lg font-bold text-secondary group-hover:text-white group-hover:-translate-x-10 transform transition-transform duration-500 ease-in-out">Назад</span>
                                    </Link>
                                </div>
                                <div className="flex flex-col md:flex-row gap-5 lg:gap-10">
                                    <div className="flex flex-col gap-2 border-b md:border-b-0 md:border-r border-secondary/12 dark:border-white/12 pb-5 md:pr-5 lg:pr-10">
                                        <span className="text-base text-secondary/70 dark:text-white/70">Виды работ</span>
                                        <p className="font-medium">{project.ScopeOfWork.join(", ")}</p>
                                    </div>
                                    <div className="flex flex-col gap-2 border-b md:border-b-0 md:border-r border-secondary/12 dark:border-white/12 pb-5 md:pr-5 lg:pr-10">
                                        <span className="text-base text-secondary/70 dark:text-white/70">Сфера</span>
                                        <p className="font-medium">{project.industry}</p>
                                    </div>
                                    <div className="flex flex-col gap-2 border-b md:border-b-0 md:border-r border-secondary/12 dark:border-white/12 pb-5 md:pr-5 lg:pr-10">
                                        <span className="text-base text-secondary/70 dark:text-white/70">Стоимость</span>
                                        <p className="font-medium">{project.raised}</p>
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <span className="text-base text-secondary/70 dark:text-white/70">Ссылка</span>
                            ...
                            <div>
                                <div className="flex flex-col xl:flex xl:flex-row items-start gap-8">
                                    <div className="flex items-center gap-4 md:gap-8 w-full max-w-xl">
                                        <h2 className="text-4xl lg:text-5xl xl:text-56">Описание</h2>
                                    </div>
